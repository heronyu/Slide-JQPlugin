	...
	$.fn.sliderFlash=function(options){
		...
		return this.each(function() {
			...
			function navSlideControl(){
						slider.find('ul').css({"transform":"translateX("+"-"+visWidth*sliderInd+"px"+")","-webkit-transform":"translateX("+"-"+visWidth*sliderInd+"px"+")","-moz-transform":"translateX("+"-"+visWidth*sliderInd+"px"+")"});
			...
			function displayNavBtn(){
				if(slider.vars.directionNav){
					slider.append('<ol class="directionNav"><li class="left"></li><li class="right"></li></ol>');
 					slider.find('ol li.left').bind('click',function(){
 						setSlideSpeed(speedSet());
 						slider.navLeft();
 					});
					slider.find('ol li.right').bind('click',function(){
						setSlideSpeed(speedSet());
						slider.navRight();
					});
				}
			...
			*/
			...
			function slideshowSlide(){
					    ...
					    if(sliderInd<elLeg){
							...
							slider.find('ul').css({"transform":"translateX("+"-"+0+"px"+")","-webkit-transform":"translateX("+"-"+0+"px"+")","-moz-transform":"translateX("+"-"+0+"px"+")"});
							slider.find('ul li').removeClass('active');
							slider.find('ul li').eq(0).addClass('active');
					    }
			...
			function slideshowFade(){
					    ...
					    if(sliderInd<elLeg){
							slider.find('ul li').eq(sliderInd).animate({opacity:'1'}).addClass('active');
							slider.find('ul li').eq(sliderInd).siblings('li').animate({opacity:'0'}).removeClass('active');
					    }else{
							slider.find('ul li').eq(0).animate({opacity:'1'}).addClass('active');
							slider.find('ul li').eq(0).siblings('li').animate({opacity:'0'}).removeClass('active');
